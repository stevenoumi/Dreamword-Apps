...
DB_HOST="192.168.56.11"  # L'adresse IP de votre machine de base de données
...
# [3] - Configuration files for phpmyadmin
...
# Configure phpMyAdmin to connect to the database on the specified host
sed -i "s|cfg\['Servers'\]\[\$i\]\['host'\] = 'localhost';|cfg['Servers'][\$i]['host'] = '$DB_HOST';|" \
  ${WWW_REP}/myadmin/config.inc.php

# Execute MySQL commands using the specified MySQL command path
...
cat <<EOF
Service installed at http://$IP/myadmin/
